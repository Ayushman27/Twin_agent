...
async def extract_voice_intent(prompt: str, history: List[Dict[str, str]]) -> Dict[str, Any]:
    ...
    for pat in MSG_FULL_PATTERNS:
        ...
        if match:
            ...
            if rec and cleaned_txt and not is_wake_word_or_self_reference(rec):
                return {"intent": "SEND_MESSAGE", "is_messaging": True, "recipient": rec, "text": cleaned_txt}

    for pat in GENERIC_MSG_PATTERNS:
        if pat.search(clean_p):
            return {"intent": "SEND_MESSAGE", "is_messaging": True, "recipient": None, "text": None}
    ...
    for pat in RECIPIENT_ONLY_PATTERNS:
        ...
        if match:
            ...
            if rec and not is_wake_word_or_self_reference(rec):
                return {"intent": "SEND_MESSAGE", "is_messaging": True, "recipient": rec, "text": None}
        ...
        for item in reversed(history):
            ...
            if m_rec:
                ...
                if rec_name and rec_name.lower() not in ["her", "him", "them"] and not is_wake_word_or_self_reference(rec_name) and cleaned_txt:
                    return {"intent": "SEND_MESSAGE", "is_messaging": True, "recipient": rec_name, "text": cleaned_txt}
    ...
    try:
        ...
        extraction_system_prompt = (
            ...
            "}\n"
            "Rules:\n"
            "- If the user is simply greeting the assistant (e.g. 'hello Echo', 'hi Eco', 'hey Echo'), set intent to CHAT.\n"
            ...
            "- If the user previously asked to send a message to a person (e.g. in history) and is now supplying the text in the current input, extract the recipient from history and text from current input.\n"
        ...
        if json_match:
            ...
            if intent_val in ["SEND_EMAIL", "SEND_MESSAGE"]:
                ...
                body = parsed.get("body") or parsed.get("text")
                ...
                return {
                    "intent": intent_val,
                    "is_messaging": (intent_val == "SEND_MESSAGE"),
                    "recipient": clean_rec,
